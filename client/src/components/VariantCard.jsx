...
const VariantCard = ({ variant, onUpdate, onDelete, totalWeight, totalScans, isLeader, isControl }) => {
    ...
    return (
        <div className={`group relative rounded-2xl p-5 transition-all border ${!isEnabled ? 'bg-slate-50/50  border-slate-200  opacity-75' : isLeader ? 'bg-indigo-50/30  border-indigo-200 ' : 'bg-white  border-slate-200  hover:border-primary/30 shadow-sm'}`}>
            {/* Status Badges Container */}
            <div className="absolute -top-3 left-4 flex items-center gap-2">
                {isEnabled && isLeader && (
                    <div className="px-2 py-1 bg-indigo-600 text-white text-[10px] font-bold uppercase tracking-widest rounded shadow-lg shadow-indigo-200  flex items-center gap-1">
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-4 py-2.5 text-sm font-medium border border-slate-300  rounded-lg bg-white  text-slate-900  focus:ring-2 focus:ring-primary/20 focus:border-primary disabled:opacity-50"
                    ...
                    <div className="flex-1 min-w-0 space-y-3">
                        ...
                        <div className="flex items-center justify-between gap-2">
                            <div className="flex items-center gap-2 min-w-0">
                                ...
                                <span className={`text-sm font-bold truncate ${isEnabled ? 'text-slate-900 ' : 'text-slate-400'}`}>
                                    {isControl ? 'Control (Original URL)' : (variant.name || variant.label || `Variant ${variant.id}`)}
                                </span>
                            </div>
                            {isEnabled && totalScans > 0 && (
                                <div className="flex items-center gap-1.5 px-2 py-0.5 rounded bg-emerald-50  text-emerald-600  text-[10px] font-bold uppercase whitespace-nowrap">
                                    {scanShare}% Share
                                </div>
                            )}
                        </div>
                        ...
                        <div className="space-y-1.5">
                            <div className="flex items-center justify-between">
                                ...
                                <span className={`text-sm font-bold ${isEnabled ? 'text-slate-900 ' : 'text-slate-400'}`}>{isEnabled ? variant.weight : 0}%</span>
                            </div>
                        ...
                        <div className="flex items-center gap-4 pt-1">
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className={`text-sm font-bold ${isEnabled ? 'text-slate-700 ' : 'text-slate-400'}`}>
                    ...
                    <div className="flex-shrink-0 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        {!isControl && (
                            <button
                                ...
                                className={`p-2 rounded-lg transition-colors ${isEnabled ? 'text-emerald-500 hover:bg-emerald-50 :bg-emerald-900/10' : 'text-slate-400 hover:bg-slate-100 :bg-slate-800'}`}
                        ...
                        <button
                            ...
                            className="p-2 text-slate-400 hover:text-primary hover:bg-slate-100 :bg-slate-800 rounded-lg transition-colors"
                            title="Edit"
                        >
