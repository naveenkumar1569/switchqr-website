...
const LockedFeature = ({ feature, requiredPlan, description, className = '' }) => {
    ...
    return (
        <div className={`flex flex-col gap-4 rounded-xl bg-white  p-6 border border-slate-200  shadow-sm ${className}`}>
            <div className="flex items-start gap-4">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary">
                    <span className="material-symbols-outlined text-2xl">lock</span>
                </div>
                <div className="flex-1 min-w-0">
                    <p className="font-bold text-slate-900  text-base mb-1">{feature}</p>
                    {description && (
